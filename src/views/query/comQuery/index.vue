    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="Plus" @click="handleAdd"
          v-hasPermi="['query:comQuery:add']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate"
          v-hasPermi="['query:comQuery:edit']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete"
          v-hasPermi="['query:comQuery:remove']">删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="Download" @click="handleExport"
          v-hasPermi="['query:comQuery:export']">导出</el-button>
      </el-col>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="comQueryRef" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="sql语句" prop="customSql">
          <el-input class="mb8" v-model="form.customSql" type="textarea" placeholder="请输入内容" />
          <el-button type="primary" size="mini" @click="handleTest(form.customSql)">测试</el-button>
        </el-form-item>
        <el-form-item label="类型" prop="type">
          <el-select v-model="form.type" placeholder="请选择类型">
            <el-option v-for="dict in sys_query_type" :key="dict.value" :label="dict.label"
...
import { listComQuery, getComQuery, delComQuery, addComQuery, updateComQuery, testBizData } from "@/api/query/comQuery";
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除通用查询编号为"' + _ids + '"的数据项？').then(function () {
...
function handleExport() {
  proxy.download('query/comQuery/export', {
    ...queryParams.value
