<template>
    <div class="app-container">
        ...
        <el-table v-if="refreshTable" v-loading="loading" :data="materialCategoryList" row-key="id"
        ...
        <el-dialog :title="title" v-model="open" width="500px" append-to-body>
            <el-form ref="materialCategoryRef" :model="form" :rules="rules" label-width="80px">
                <el-form-item label="父级分类" prop="parentId">
                    <el-tree-select v-model="form.parentId" :data="materialCategoryOptions"
                        :props="{ value: 'id', label: 'name', children: 'children' }" value-key="id"
                        placeholder="请选择父级分类编号" check-strictly />
                </el-form-item>
                <el-form-item label="分类编码" prop="code">
...
import { listMaterialCategory, getMaterialCategory, delMaterialCategory, addMaterialCategory, updateMaterialCategory } from "@/api/material/materialCategory";
...
function handleDelete(row) {
    proxy.$modal.confirm('是否确认删除物料分类编号为"' + row.id + '"的数据项？').then(function () {
